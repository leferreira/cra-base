...
public class ProcessadorArquivo extends Processador {
	...
	if (getFile() != null) {
	    ...
	    logger.info("Fim do processamento do arquivoFisico " + getFile().getClientFileName() + " do usuário "
		    + getUsuario().getLogin());

	    return getArquivo();

	} else {
	    throw new InfraException("O arquivo " + getFile().getClientFileName()
		    + " enviado não pode ser processado.");
	}
    }
	...
	logger.info("Início do criação de Arquivo TXT" + getArquivo().getNomeArquivo() + " do usuário "
		+ getUsuario().getLogin());
	...
	logger.info("Fim da criação de Arquivo TXT" + getArquivo().getNomeArquivo() + " do usuário "
		+ getUsuario().getLogin());
	...
	logger.info("Início do criação de Arquivo DP TXT" + getArquivo().getNomeArquivo() + " do usuário "
		+ getUsuario().getLogin());
	verificaDiretorio();
	setArquivoFisico(new File(getPathUsuarioTemp() + ConfiguracaoBase.BARRA + remessa.getId()));
	fabricaDeArquivo.processarArquivoPersistenteDesistenciaProtesto(remessa, getArquivoFisico(), getErros());

	logger.info("Fim da criação de Arquivo DP TXT" + getArquivo().getNomeArquivo() + " do usuário "
		+ getUsuario().getLogin());
	...
	logger.info("Início do criação de Arquivo CP TXT" + getArquivo().getNomeArquivo() + " do usuário "
		+ getUsuario().getLogin());
	verificaDiretorio();
	setArquivoFisico(new File(getPathUsuarioTemp() + ConfiguracaoBase.BARRA + remessa.getId()));
	fabricaDeArquivo.processarArquivoPersistenteCancelamentoProtesto(remessa, getArquivoFisico(), getErros());

	logger.info("Fim da criação de Arquivo CP TXT" + getArquivo().getNomeArquivo() + " do usuário "
		+ getUsuario().getLogin());
	...
	logger.info("Início do criação de Arquivo AC TXT" + getArquivo().getNomeArquivo() + " do usuário "
		+ getUsuario().getLogin());
	verificaDiretorio();
	setArquivoFisico(new File(getPathUsuarioTemp() + ConfiguracaoBase.BARRA + remessa.getId()));
	fabricaDeArquivo.processarArquivoPersistenteAutorizacaoCancelamentoProtesto(remessa, getArquivoFisico(), getErros());

	logger.info("Fim da criação de Arquivo AC TXT" + getArquivo().getNomeArquivo() + " do usuário "
		+ getUsuario().getLogin());
	...
	try {
	    if (getArquivoFisico().renameTo(new File(getPathUsuario() + ConfiguracaoBase.BARRA
		    + getArquivo().getId()))) {
	...
	logger.info("Fim validação do arquivoFisico " + getFile().getClientFileName() + " enviado pelo usuário "
		+ getUsuario().getLogin());
    }
	...
	try {
	    ...
	    throw new InfraException("Não foi possível criar arquivo Físico temporário para o arquivo "
		    + getFile().getClientFileName());
	}
    }
