	...
	public void removerRemessa(Instituicao instituicao, Arquivo arquivo) {
		...
		try {
			for (Remessa r : arquivo.getRemessas()) {
				query = createSQLQuery("DELETE FROM tb_anexo " + "WHERE titulo_id in (" + "SELECT tit.id_titulo FROM tb_titulo AS tit "
						+ "WHERE tit.id_titulo in ( SELECT anexo.titulo_id FROM tb_anexo AS anexo ) " + "AND tit.remessa_id= " + r.getId() + ")");
				query.executeUpdate();
