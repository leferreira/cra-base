...
public enum CodigoErro {
    ...
    CRA_SERVICO_INDISPONIVEL("9999", "Serviço temporariamente indisponível."), CRA_SUCESSO("0000", "Envio efetuado com sucesso."), //
    ...
    CRA_ARQUIVO_CONTEM_CARACTERE_QUE_NAO_ESTA_NO_PADRAO_ASCII(" 2101",
    ...
    CRA_APRESENTANTE_NAO_AUTORIZADO_A_ENVIAR_TITULOS_PARA_O_MUNICIPIO(" 2105",
    ...
    CRA_ARQUIVO_CORROMPIDO_NUMERO_DE_CONTROLE_DO_DEVEDOR_NAO_ESTA_CONTINUO(" 2107",
            "Arquivo corrompido. Número de controle do devedor (XXX) não está contínuo."), //
    CRA_ARQUIVO_CORROMPIDO_SOMA_DE_TITULOS_EXISTENTES_NO_ARQUIVO_NAO_CONFERE_COM_TOTAL_INFORMADO_NO_HEADER(" 2108",
            "Arquivo corrompido. Soma de títulos existentes no arquivo (XXX) não confere com total informado no header (XXX)."), //
    CRA_ARQUIVO_CORROMPIDO_SOMA_DE_INDICACOES_EXISTENTES_NO_ARQUIVO_NAO_CONFERE_COM_TOTAL_INFORMADO_NO_HEADER(" 2109",
            "Arquivo corrompido. Soma de indicações existentes no arquivo (XXX) não confere com total informado no header (XXX)."), //
    CRA_ARQUIVO_CORROMPIDO_SOMA_DE_ORIGINAIS_EXISTENTES_NO_ARQUIVO_NAO_CONFERE_COM_TOTAL_INFORMADO_NO_HEADER(" 2110",
    ...
    CRA_ARQUIVO_CORROMPIDO_NUMERO_SEQUENCIAL_DO_REGISTRO_NO_HEADER_NAO_ESTA_CONTINUO_FALTOU_REGISTRO(" 2114",
            "Arquivo corrompido. Número sequencial do registro no header (XXX) não está contínuo. Faltou registro (XXX)."), //
    CRA_ARQUIVO_NAO_E_DE_REMESSA(" 2115", "Arquivo não é de remessa (tipo TPR). "), CRA_O_ARQUIVO_JA_FOI_ENVIADO_EM(" 2116",
            "O arquivo XXX já foi enviado em XXX."), CRA_JA_FOI_ENVIADA_REMESSA_COM_O_MESMO_SEQUENCIAL_PARA_O_MUNICIPIO(" 2117",
                    "Já foi enviada remessa com o mesmo sequencial (XXX) para o município (XXX)."), //
    CRA_CODIGO_FEBRABAN_DO_APRESENTANTE_INVALIDO(" 2118",
            "Código Febraban do apresentante inválido (XXX)."), CRA_EXISTE_REMESSA_NO_ARQUIVO_COM_O_MESMO_SEQUENCIAL_PARA_O_MUNICIPIO(
                    " 2119", "Existe remessa no arquivo com o mesmo sequencial (XXX) para o município (XXX)."), //
    CRA_ARQUIVO_CORROMPIDO_NUMERO_SEQUENCIAL_DO_REGISTRO_NA_TRANSACAO_NAO_ESTA_CONTINUO_FALTOU_REGISTRO(" 2120",
    ...
    CRA_ARQUIVO_CORROMPIDO_SOMA_DE_REGISTROS_EXISTENTES_NO_ARQUIVO_NAO_CONFERE_COM_TOTAL_INFORMADO_NO_HEADER_DO_ARQUIVO(" 2129",
            "Arquivo corrompido. Soma de registros existentes no arquivo (XXX) não confere com total informado no header do arquivo (XXX)."), CRA_CODIGO_DO_MUNICIPIO_NAO_INFORMADO(
                    " 2130", "Código do município não informado."), //
    CRA_NAO_EXISTE_MUNICIPIO_COM_O_CODIGO_INFORMADO_NA_CRA(" 2131",
            "Não existe município com o código informado (XXX) na CRA."), CRA_CARTORIO_NAO_ENCONTRADO_NO_MUNICIPIO(" 2132",
                    "Cartório (XXX) não encontrado no município (XXX - XXX)."), //
    CRA_ARQUIVO_CORROMPIDO_SOLICITAÇÃO_NAO_E_DE_CANCELAMENTO(" 2133",
            "Arquivo corrompido. Solicitação não é de cancelamento (C)."), CRA_PROTOCOLO_INVALIDO("2134",
                    "Protocolo inválido (XXX)."), CRA_DATA_DO_PROTOCOLO_INVALIDA(" 2135", "Data do protocolo inválida (XXX)."), //
    CRA_ARQUIVO_CORROMPIDO_SOMATORIO_DE_SEGURANCA_DOS_REGISTROS_NAO_CONFERE_COM_A_SOMA_DAS_QUANTIDADES_INFORMADAS_NO_HEADER(" 2136",
            "Arquivo corrompido. Somatório de segurança dos registros (XXX) não confere com a soma das quantidades informadas no header (XXX)."), CRA_CAMPO_INVALIDO(
                    " 2137", "Campo 'XXX' inválido."), //
    CRA_DATA_DO_MOVIMENTO_INVALIDA(" 2138", "Data do movimento inválida (XXX). "), CRA_JA_PASSOU_DO_HORÁRIO_LIMITE_PARA_ENVIO_DA_REMESSA(
            " 2139", "Já passou do horário limite para envio da remessa."), CRA_JA_PASSOU_DO_HORÁRIO_LIMITE_PARA_ENVIO_DO_CANCELAMENTO(
    ...
    CRA_JA_PASSOU_DO_HORÁRIO_LIMITE_PARA_ENVIO_DA_DESISTENCIA("2141",
            "Já passou do horário limite para envio da desistência."), CRA_DADOS_DE_ENVIO_INVALIDOS("0003", "Dados de envio inválidos"), //
    ...
    CARTORIO_RECEBIMENTO_DESISTENCIA_CANCELAMENTO_COM_SUCESSO("0000",
            "Arquivo de Desistência/Cancelamento confirmado o recebimento com sucesso.");//
