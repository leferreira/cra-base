...
public class FabricaDeArquivoXML extends AbstractFabricaDeArquivo {
    ...
    public Arquivo converter(File arquivoFisico, Arquivo arquivo, List<Exception> erros) {
        if (TipoArquivoEnum.REMESSA.equals(arquivo.getTipoArquivo().getTipoArquivo())) {
            ...
            logger.error("Não é possível o envio de arquivos do tipo Cancelamento de Protesto por esse meio.");
            throw new InfraException("Não é possível o envio de arquivos do tipo Cancelamento de Protesto por esse meio.");

        } else if (TipoArquivoEnum.AUTORIZACAO_DE_CANCELAMENTO.equals(arquivo.getTipoArquivo().getTipoArquivo())) {
            logger.error("Não é possível o envio de arquivos do tipo Autorização de Cancelamento por esse meio.");
            throw new InfraException("Não é possível o envio de arquivos do tipo Autorização de Cancelamento por esse meio.");
        }
        return null;
    }
