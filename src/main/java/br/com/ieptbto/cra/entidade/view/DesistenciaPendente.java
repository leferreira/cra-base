...
@NamedNativeQueries({
	@NamedNativeQuery(name="findDesistenciasPendentes", 
			query="select * from view_desistencia_pendente order by iddesistencia_desistenciaprotesto asc", resultClass=DesistenciaPendente.class),
	@NamedNativeQuery(name="findDesistenciasPendentesCartorio", 
			query="select * from view_desistencia_pendente where idinstituicao_cartorio= :id order by iddesistencia_desistenciaprotesto asc", 
			resultClass=DesistenciaPendente.class),
	@NamedNativeQuery(name="findDesistenciasPendentesInstituicao", 
			query="select * from view_desistencia_pendente where idinstituicao_instituicao= :id order by iddesistencia_desistenciaprotesto asc", 
			resultClass=DesistenciaPendente.class)
})
public class DesistenciaPendente extends ViewArquivoPendente {
	
	private static final long serialVersionUID = 1L;

	private Integer idDesistencia_DesistenciaProtesto;
	...
	@Id
	public Integer getIdDesistencia_DesistenciaProtesto() {
		return idDesistencia_DesistenciaProtesto;
	}
	...
	public void setIdDesistencia_DesistenciaProtesto(Integer idDesistencia_DesistenciaProtesto) {
		this.idDesistencia_DesistenciaProtesto = idDesistencia_DesistenciaProtesto;
	}
